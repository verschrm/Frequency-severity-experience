...
##### Load data #####
### Load results from MTPL_Bayesian_HMM_Sparse.R

##### Predictions K=1 #####
### Estimated prior parameters
cbind(Optim_1$theta_N[ncol(MTPL_RF_Freq) + 1], Optim_1$theta_N[ncol(MTPL_RF_Freq) + 2],
      Optim_1$theta_X[1 + ncol(MTPL_RF_Sev) + 1], Optim_1$theta_X[1 + ncol(MTPL_RF_Sev) + 2])
...
lamb_1 <- pmin(pmax(MTPL_Exp_Freq * Optim_1$theta_N[length(Optim_1$theta_N) - 1] / 
                      tail(Optim_1$theta_N, 1) * exp(MTPL_RF_Freq %*% Optim_1$theta_N[
                        1:ncol(MTPL_RF_Freq)]), LB_eps), 1e+15) / MTPL_Exp_Freq
mu_phi_1 <- pmin(pmax(Optim_1$theta_X[1] * tail(Optim_1$theta_X, 1) / (
  Optim_1$theta_X[length(Optim_1$theta_X) - 1] - 1) * exp(MTPL_RF_Sev_Freq %*% Optim_1$theta_X[
    2:(1 + ncol(MTPL_RF_Sev))]), LB_eps), 1e+15) / Optim_1$theta_X[1]
# Posterior parameters
expo_lamb_1 <- pmin(pmax(MTPL_Exp_Freq * exp(MTPL_RF_Freq %*% Optim_1$theta_N[
  1:ncol(MTPL_RF_Freq)]), LB_eps), 1e+15)
mu_1 <- pmin(pmax(Optim_1$theta_X[1] * exp(MTPL_RF_Sev_Freq %*% Optim_1$theta_X[
  2:(1 + ncol(MTPL_RF_Sev))]), LB_eps), 1e+15)
...
lamb_post_1 <- (Optim_1$theta_N[length(Optim_1$theta_N) - 1] + Update_alpha_U_1) /
  (tail(Optim_1$theta_N, 1) + Update_beta_U_1) * exp(MTPL_RF_Freq %*% Optim_1$theta_N[
    1:ncol(MTPL_RF_Freq)])
mu_phi_post_1 <- (tail(Optim_1$theta_X, 1) + Update_beta_V_1) /
  (Optim_1$theta_X[length(Optim_1$theta_X) - 1] - 1 + Update_alpha_V_1) *
  exp(MTPL_RF_Sev_Freq %*% Optim_1$theta_X[2:(1 + ncol(MTPL_RF_Sev))])
...
##### Predictions K=2 #####
### Estimated prior parameters
cbind(Optim_2$theta_N[ncol(MTPL_RF_Freq) + 1:2], Optim_2$theta_N[ncol(MTPL_RF_Freq) + 3:4],
      Optim_2$theta_X[1 + ncol(MTPL_RF_Sev) + 1:2], Optim_2$theta_X[1 + ncol(MTPL_RF_Sev) + 3:4])
...
lamb_2 <- pmin(pmax(MTPL_Exp_Freq * exp(MTPL_RF_Freq %*% Optim_2$theta_N[
  1:ncol(MTPL_RF_Freq)]) %*% (Optim_2$theta_N[(ncol(MTPL_RF_Freq) + 1):(
    ncol(MTPL_RF_Freq) + 2)] / Optim_2$theta_N[(ncol(MTPL_RF_Freq) + 1 + 2):(
      ncol(MTPL_RF_Freq) + 2 * 2)]), LB_eps), 1e+15) / MTPL_Exp_Freq
mu_phi_2 <- pmin(pmax(Optim_2$theta_X[1] * exp(MTPL_RF_Sev_Freq %*% Optim_2$theta_X[
  2:(1 + ncol(MTPL_RF_Sev))]) %*% (Optim_2$theta_X[(2 + ncol(MTPL_RF_Sev) + 2):(
    1 + ncol(MTPL_RF_Sev) + 2 * 2)] / (Optim_2$theta_X[(2 + ncol(MTPL_RF_Sev)):(
      1 + ncol(MTPL_RF_Sev) + 2)] - 1)), LB_eps), 1e+15) / Optim_2$theta_X[1]
# Posterior
expo_lamb_2 <- pmin(pmax(MTPL_Exp_Freq * exp(MTPL_RF_Freq %*% Optim_2$theta_N[
  1:ncol(MTPL_RF_Freq)]), LB_eps), 1e+15)
mu_2 <- pmin(pmax(Optim_2$theta_X[1] * exp(MTPL_RF_Sev_Freq %*% Optim_2$theta_X[
  2:(1 + ncol(MTPL_RF_Sev))]), LB_eps), 1e+15)
Update_alpha_U_2 <- rep(0, nrow(MTPL_RF_Freq))
Update_beta_U_2 <- rep(0, nrow(MTPL_RF_Freq))
Update_alpha_V_2 <- rep(0, nrow(MTPL_RF_Freq))
Update_beta_V_2 <- rep(0, nrow(MTPL_RF_Freq))
alpha_N_2 <- rep(0, nrow(MTPL_RF_Freq))
alpha_N_2[unique(MTPL_Ind_i_t)] <- as.vector(aggregate(
...
lamb_post_2 <- as.vector(exp(MTPL_RF_Freq %*% Optim_2$theta_N[1:ncol(MTPL_RF_Freq)])) * 
  sapply(1:2, function(j) (Optim_2$theta_N[ncol(MTPL_RF_Freq) + j] + Update_alpha_U_2) /
           (Optim_2$theta_N[ncol(MTPL_RF_Freq) + j + 2] + Update_beta_U_2))
mu_phi_post_2 <- as.vector(exp(MTPL_RF_Sev_Freq %*% Optim_2$theta_X[2:(1 + ncol(MTPL_RF_Sev))])) * 
  sapply(1:2, function(j) (Optim_2$theta_X[1 + j + ncol(MTPL_RF_Sev) + 2] + Update_beta_V_2) /
           (Optim_2$theta_X[1 + j + ncol(MTPL_RF_Sev)] - 1 + Update_alpha_V_2))

### Profile assignment probabilities
# Prior
Prob_Z_prior_2 <- matrix(NA, nrow = nrow(MTPL_RF_Freq), ncol = 2)
...
##### Predictions K=3 #####
### Estimated prior parameters
cbind(Optim_3$theta_N[ncol(MTPL_RF_Freq) + 1:3], Optim_3$theta_N[ncol(MTPL_RF_Freq) + 4:6],
      Optim_3$theta_X[1 + ncol(MTPL_RF_Sev) + 1:3], Optim_3$theta_X[1 + ncol(MTPL_RF_Sev) + 4:6])
...
lamb_3 <- pmin(pmax(MTPL_Exp_Freq * exp(MTPL_RF_Freq %*% Optim_3$theta_N[
  1:ncol(MTPL_RF_Freq)]) %*% (Optim_3$theta_N[(ncol(MTPL_RF_Freq) + 1):(
    ncol(MTPL_RF_Freq) + 3)] / Optim_3$theta_N[(ncol(MTPL_RF_Freq) + 1 + 3):(
      ncol(MTPL_RF_Freq) + 2 * 3)]), LB_eps), 1e+15) / MTPL_Exp_Freq
mu_phi_3 <- pmin(pmax(Optim_3$theta_X[1] * exp(MTPL_RF_Sev_Freq %*% Optim_3$theta_X[
  2:(1 + ncol(MTPL_RF_Sev))]) %*% (Optim_3$theta_X[(2 + ncol(MTPL_RF_Sev) + 3):(
    1 + ncol(MTPL_RF_Sev) + 2 * 3)] / (Optim_3$theta_X[(2 + ncol(MTPL_RF_Sev)):(
      1 + ncol(MTPL_RF_Sev) + 3)] - 1)), LB_eps), 1e+15) / Optim_3$theta_X[1]
# Posterior
expo_lamb_3 <- pmin(pmax(MTPL_Exp_Freq * exp(MTPL_RF_Freq %*% Optim_3$theta_N[
  1:ncol(MTPL_RF_Freq)]), LB_eps), 1e+15)
mu_3 <- pmin(pmax(Optim_3$theta_X[1] * exp(MTPL_RF_Sev_Freq %*% Optim_3$theta_X[
  2:(1 + ncol(MTPL_RF_Sev))]), LB_eps), 1e+15)
Update_alpha_U_3 <- rep(0, nrow(MTPL_RF_Freq))
Update_beta_U_3 <- rep(0, nrow(MTPL_RF_Freq))
Update_alpha_V_3 <- rep(0, nrow(MTPL_RF_Freq))
Update_beta_V_3 <- rep(0, nrow(MTPL_RF_Freq))
alpha_N_3 <- rep(0, nrow(MTPL_RF_Freq))
alpha_N_3[unique(MTPL_Ind_i_t)] <- as.vector(aggregate(
...
lamb_post_3 <- as.vector(exp(MTPL_RF_Freq %*% Optim_3$theta_N[1:ncol(MTPL_RF_Freq)])) * 
  sapply(1:3, function(j) (Optim_3$theta_N[ncol(MTPL_RF_Freq) + j] + Update_alpha_U_3) /
           (Optim_3$theta_N[ncol(MTPL_RF_Freq) + j + 3] + Update_beta_U_3))
mu_phi_post_3 <- as.vector(exp(MTPL_RF_Sev_Freq %*% Optim_3$theta_X[2:(1 + ncol(MTPL_RF_Sev))])) * 
  sapply(1:3, function(j) (Optim_3$theta_X[1 + j + ncol(MTPL_RF_Sev) + 3] + Update_beta_V_3) /
           (Optim_3$theta_X[1 + j + ncol(MTPL_RF_Sev)] - 1 + Update_alpha_V_3))

### Profile assignment probabilities
# Prior
Prob_Z_prior_3 <- matrix(NA, nrow = nrow(MTPL_RF_Freq), ncol = 3)
...
##### Predictions K=4 #####
### Estimated prior parameters
cbind(Optim_4$theta_N[ncol(MTPL_RF_Freq) + 1:4], Optim_4$theta_N[ncol(MTPL_RF_Freq) + 5:8],
      Optim_4$theta_X[1 + ncol(MTPL_RF_Sev) + 1:4], Optim_4$theta_X[1 + ncol(MTPL_RF_Sev) + 5:8])
...
lamb_4 <- pmin(pmax(MTPL_Exp_Freq * exp(MTPL_RF_Freq %*% Optim_4$theta_N[
  1:ncol(MTPL_RF_Freq)]) %*% (Optim_4$theta_N[(ncol(MTPL_RF_Freq) + 1):(
    ncol(MTPL_RF_Freq) + 4)] / Optim_4$theta_N[(ncol(MTPL_RF_Freq) + 1 + 4):(
      ncol(MTPL_RF_Freq) + 2 * 4)]), LB_eps), 1e+15) / MTPL_Exp_Freq
mu_phi_4 <- pmin(pmax(Optim_4$theta_X[1] * exp(MTPL_RF_Sev_Freq %*% Optim_4$theta_X[
  2:(1 + ncol(MTPL_RF_Sev))]) %*% (Optim_4$theta_X[(2 + ncol(MTPL_RF_Sev) + 4):(
    1 + ncol(MTPL_RF_Sev) + 2 * 4)] / (Optim_4$theta_X[(2 + ncol(MTPL_RF_Sev)):(
      1 + ncol(MTPL_RF_Sev) + 4)] - 1)), LB_eps), 1e+15) / Optim_4$theta_X[1]
# Posterior
expo_lamb_4 <- pmin(pmax(MTPL_Exp_Freq * exp(MTPL_RF_Freq %*% Optim_4$theta_N[
  1:ncol(MTPL_RF_Freq)]), LB_eps), 1e+15)
mu_4 <- pmin(pmax(Optim_4$theta_X[1] * exp(MTPL_RF_Sev_Freq %*% Optim_4$theta_X[
  2:(1 + ncol(MTPL_RF_Sev))]), LB_eps), 1e+15)
Update_alpha_U_4 <- rep(0, nrow(MTPL_RF_Freq))
Update_beta_U_4 <- rep(0, nrow(MTPL_RF_Freq))
Update_alpha_V_4 <- rep(0, nrow(MTPL_RF_Freq))
Update_beta_V_4 <- rep(0, nrow(MTPL_RF_Freq))
alpha_N_4 <- rep(0, nrow(MTPL_RF_Freq))
alpha_N_4[unique(MTPL_Ind_i_t)] <- as.vector(aggregate(
...
lamb_post_4 <- as.vector(exp(MTPL_RF_Freq %*% Optim_4$theta_N[1:ncol(MTPL_RF_Freq)])) * 
  sapply(1:4, function(j) (Optim_4$theta_N[ncol(MTPL_RF_Freq) + j] + Update_alpha_U_4) /
           (Optim_4$theta_N[ncol(MTPL_RF_Freq) + j + 4] + Update_beta_U_4))
mu_phi_post_4 <- as.vector(exp(MTPL_RF_Sev_Freq %*% Optim_4$theta_X[2:(1 + ncol(MTPL_RF_Sev))])) * 
  sapply(1:4, function(j) (Optim_4$theta_X[1 + j + ncol(MTPL_RF_Sev) + 4] + Update_beta_V_4) /
           (Optim_4$theta_X[1 + j + ncol(MTPL_RF_Sev)] - 1 + Update_alpha_V_4))

### Profile assignment probabilities
# Prior
Prob_Z_prior_4 <- matrix(NA, nrow = nrow(MTPL_RF_Freq), ncol = 4)
...
# Input
K <- 2
size <- sapply(1:K, function(j) Optim_2$theta_N[ncol(MTPL_RF_Freq) + j])
prob <- sapply(1:K, function(j) (Optim_2$theta_N[ncol(MTPL_RF_Freq) + j + 2]) / (
  Optim_2$theta_N[ncol(TPL_RF_Freq) + j + 2] + lamb_2[, j]))
shape1 <- 1
shape2 <- sapply(1:K, function(j) as.vector(exp(MTPL_RF_Sev_Freq %*% Optim_2$theta_X[2:(
  1 + ncol(MTPL_RF_Sev))])))
shape3 <- sapply(1:K, function(j) Optim_2$theta_X[1 + j + ncol(MTPL_RF_Sev)])
scale <- sapply(1:K, function(j) (Optim_2$theta_X[1 + j + ncol(MTPL_RF_Sev) + 2]) / Optim_2$theta_X[1])
Prob_Z <- Prob_Z_prior_2
# Output
rho_prior_2 <- c()
tau_prior_2 <- c()
for (i in 1:nrow(Prob_Z)) {
  dep_i <- rho_tau(K = K, size = as.vector(size), prob = as.vector(prob[i, ]), shape1 = shape1,
                   shape2 = as.vector(shape2[i, ]), shape3 = as.vector(shape3), 
...
### K = 2 (posterior)
K <- 2
size <- sapply(1:K, function(j) Optim_2$theta_N[ncol(MTPL_RF_Freq) + j] + Update_alpha_U_2)
prob <- sapply(1:K, function(j) (Optim_2$theta_N[ncol(MTPL_RF_Freq) + j + 2] + Update_beta_U_2) / (
  (Optim_2$theta_N[ncol(MTPL_RF_Freq) + j + 2] + Update_beta_U_2) + lamb_2[, j]))
shape1 <- 1
shape2 <- sapply(1:K, function(j) as.vector(exp(MTPL_RF_Sev_Freq %*% Optim_2$theta_X[2:(
  1 + ncol(MTPL_RF_Sev))])))
shape3 <- sapply(1:K, function(j) Optim_2$theta_X[1 + j + ncol(MTPL_RF_Sev)] + Update_alpha_V_2)
scale <- sapply(1:K, function(j) 
  (Optim_2$theta_X[1 + j + ncol(MTPL_RF_Sev) + 2] + Update_beta_V_2) / Optim_2$theta_X[1])
Prob_Z <- Prob_Z_post_2
# Output
rho_post_2 <- c()
tau_post_2 <- c()
for (i in 1:nrow(Prob_Z)) {
  dep_i <- rho_tau(K = K, size = as.vector(size[i, ]), prob = as.vector(prob[i, ]), shape1 = shape1, 
                   shape2 = as.vector(shape2[i, ]), shape3 = as.vector(shape3[i, ]), 
...
### K = 3 (prior)
K <- 3
size <- sapply(1:K, function(j) Optim_3$theta_N[ncol(MTPL_RF_Freq) + j])
prob <- sapply(1:K, function(j) (Optim_3$theta_N[ncol(MTPL_RF_Freq) + j + 2]) / (
  Optim_3$theta_N[ncol(TPL_RF_Freq) + j + 2] + lamb_3[, j]))
shape1 <- 1
shape2 <- sapply(1:K, function(j) as.vector(exp(MTPL_RF_Sev_Freq %*% Optim_3$theta_X[2:(
  1 + ncol(MTPL_RF_Sev))])))
shape3 <- sapply(1:K, function(j) Optim_3$theta_X[1 + j + ncol(MTPL_RF_Sev)])
scale <- sapply(1:K, function(j) (Optim_3$theta_X[1 + j + ncol(MTPL_RF_Sev) + 2]) / Optim_3$theta_X[1])
...
### K = 3 (posterior)
K <- 3
size <- sapply(1:K, function(j) Optim_3$theta_N[ncol(MTPL_RF_Freq) + j] + Update_alpha_U_3)
prob <- sapply(1:K, function(j) (Optim_3$theta_N[ncol(MTPL_RF_Freq) + j + 2] + Update_beta_U_3) / (
  (Optim_3$theta_N[ncol(MTPL_RF_Freq) + j + 2] + Update_beta_U_3) + lamb_3[, j]))
shape1 <- 1
shape2 <- sapply(1:K, function(j) as.vector(exp(MTPL_RF_Sev_Freq %*% Optim_3$theta_X[2:(
  1 + ncol(MTPL_RF_Sev))])))
shape3 <- sapply(1:K, function(j) Optim_3$theta_X[1 + j + ncol(MTPL_RF_Sev)] + Update_alpha_V_3)
scale <- sapply(1:K, function(j) 
  (Optim_3$theta_X[1 + j + ncol(MTPL_RF_Sev) + 2] + Update_beta_V_3) / Optim_3$theta_X[1])
