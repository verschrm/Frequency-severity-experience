...
### Forward-Backward algorithm with the BFGS method
### to maximize the frequency-severity log-likelihood 
### based on a dynamic latent risk type decomposition. 
...
# theta_Z_init = Initial parameter matrix for Z;
# theta_N_init = Initial parameter vector for N;
# theta_X_init = Initial parameter vector for X;
# K            = The number of latent risk types
#                to consider;
...
# Moreover, note that theta_N assumes that its
# elements represent (delta_A, a_U, b_U) and 
# that theta_X assumes that its elements represent 
# (phi, delta_B, a_V, b_V). Finally, scaling of the
# Baum-Welch algorithm increases its numerical 
# stability, especially in case of a large number of 
# periods.

##### Function EB_BW_BFGS_Sparse() #####
EB_BW_BFGS_Sparse <- function(theta_Z_init, theta_N_init, theta_X_init, 
  ...
  Q_N_Sparse <- dget('.../Sparse/Functions/Q_N_Sparse.R')
  Q_X_Sparse <- dget('.../Sparse/Functions/Q_X_Sparse.R')
  # Functions for computing the gradient vector of the log-likelihoods
  g_N_Sparse <- dget('.../Sparse/Functions/g_N_Sparse.R')
  g_X_Sparse <- dget('.../Sparse/Functions/g_X_Sparse.R')
  # Functions for computing the Hessian matrix of the log-likelihoods
  H_N_Sparse <- dget('.../Sparse/Functions/H_N_Sparse.R')
  H_X_Sparse <- dget('.../Sparse/Functions/H_X_Sparse.R')
  ...
  if (K > 1) {
    ...
    a_U <- as.vector(theta_N[(ncol(RF_N) + 1):(ncol(RF_N) + K)])
    b_U <- as.vector(theta_N[(ncol(RF_N) + 1 + K):(ncol(RF_N) + 2 * K)])
    phi <- as.numeric(theta_X[1])
    delta_B <- theta_X[2:(1 + ncol(RF_X))]
    a_V <- as.vector(theta_X[(2 + ncol(RF_X)):(1 + ncol(RF_X) + K)])
    b_V <- as.vector(theta_X[(2 + ncol(RF_X) + K):(1 + ncol(RF_X) + 2 * K)])
    ...
    Prob_N <- sapply(1:K, function(j) 
      dnbinom(x = Counts, size = a_U[j], 
              prob = pmax(b_U[j] / (b_U[j] + Expo_lambda), LB_eps), 
              log = FALSE))
    # Calculate claim size probability matrix
    Prob_X <- sapply(1:K, function(j)
      dbetapr(x = Sizes, shape1 = mu, shape2 = a_V[j], 
              scale = max(b_V[j] / phi, LB_eps), log = FALSE))
    # Determine joint response, or emmission, probabilities
    ...
    while ((Tol_abs > Tol_abs_BW) & (Tol_rel > Tol_rel_BW) & (Iter <= Iter_BW)) {
      ...
      M_Q <- maxBFGS(fn = function(theta) Q_N_Sparse(theta_N = theta, Prob_Z_hat = Prob_Z_hat, 
                                                     Counts = Counts, Expo = Expo, RF_N = RF_N, LB_eps = LB_eps,
                                                     penalty = penalty),
                     grad = function(theta) g_N_Sparse(theta_N = theta, Prob_Z_hat = Prob_Z_hat, 
                                                       Counts = Counts, Expo = Expo, RF_N = RF_N, LB_eps = LB_eps,
                                                       penalty = penalty), 
                     hess = function(theta) H_N_Sparse(theta_N = theta, Prob_Z_hat = Prob_Z_hat, 
                                                       Counts = Counts, Expo = Expo, RF_N = RF_N, LB_eps = LB_eps,
                                                       penalty = penalty), 
                     start = theta_N, control = list(tol = Tol_abs_M, reltol = Tol_rel_M, 
                                                     iterlim = Iter_M, printLevel = 0), 
                     finalHessian = FALSE, constraints = list(ineqA = cbind(
                       matrix(0, nrow = 2 * K, ncol = ncol(RF_N)), diag(1, nrow = 2 * K, ncol = 2 * K)),
                       ineqB = rep(0, 2 * K)))
      # Store the resulting solution
      Iter_N_r[Iter + 1] <- as.numeric(M_Q$iterations[1])
      Conv_N_r[Iter + 1] <- M_Q$code
      theta_N <- M_Q$estimate
      # Ensure constraints are not violated
      theta_N[(ncol(RF_N) + 1):(ncol(RF_N) + 2 * K)] <- pmax(theta_N[(ncol(RF_N) + 1):(ncol(RF_N) + 2 * K)], LB_eps)
      
      ### M-step for individual claim sizes
      # Optimize complete expected log-likelihood using the BFGS method
      M_Q <- maxBFGS(fn = function(theta) Q_X_Sparse(theta_X = theta, Prob_Z_hat = Prob_Z_hat[Ind_i_t, ], 
                                                     Sizes = Sizes, RF_X = RF_X, LB_eps = LB_eps,
                                                     penalty = penalty),
                     grad = function(theta) g_X_Sparse(theta_X = theta, Prob_Z_hat = Prob_Z_hat[Ind_i_t, ], 
                                                       Sizes = Sizes, RF_X = RF_X, LB_eps = LB_eps,
                                                       penalty = penalty), 
                     hess = function(theta) H_X_Sparse(theta_X = theta, Prob_Z_hat = Prob_Z_hat[Ind_i_t, ], 
                                                       Sizes = Sizes, RF_X = RF_X, LB_eps = LB_eps,
                                                       penalty = penalty), 
                     start = theta_X, control = list(tol = Tol_abs_M, reltol = Tol_rel_M, 
                                                     iterlim = Iter_M, printLevel = 0), 
                     finalHessian = FALSE, constraints = list(ineqA = rbind(c(1, rep(0, length(theta_X) - 1)), cbind(
                       matrix(0, nrow = 2 * K, ncol = 1 + ncol(RF_X)), diag(1, nrow = 2 * K, ncol = 2 * K))),
                       ineqB = c(0, rep(-1, K), rep(0, K))))
      # Store the resulting solution
      Iter_X_r[Iter + 1] <- as.numeric(M_Q$iterations[1])
      Conv_X_r[Iter + 1] <- M_Q$code
      theta_X <- M_Q$estimate
      # Ensure constraints are not violated
      theta_X[c(1, (2 + ncol(RF_X) + K):(1 + ncol(RF_X) + 2 * K))] <- pmax(theta_X[c(1, (2 + ncol(RF_X) + K):(1 + ncol(RF_X) + 2 * K))], LB_eps)
      theta_X[(2 + ncol(RF_X)):(1 + ncol(RF_X) + K)] <- pmax(theta_X[(2 + ncol(RF_X)):(1 + ncol(RF_X) + K)], 1 + LB_eps)
    ...
    # Claim counts
    Hessian <- H_N(theta_N = theta_N, Prob_Z_hat = Prob_Z_hat, 
                   Counts = Counts, Expo = Expo, RF_N = RF_N, LB_eps = LB_eps)
    V_theta_N <- try(diag(solve(-Hessian)), silent = TRUE)
    if ((inherits(V_theta_N, 'try-error')) | (sum(V_theta_N < 0) > 0)) {
      V_theta_N <- diag(ginv(-Hessian))
    }
    # Individual claim sizes
    Hessian <- H_X(theta_X = theta_X, Prob_Z_hat = Prob_Z_hat[Ind_i_t, ], 
                   Sizes = Sizes, RF_X = RF_X, LB_eps = LB_eps)
    V_theta_X <- try(diag(solve(-Hessian)), silent = TRUE)
    if ((inherits(V_theta_X, 'try-error')) | (sum(V_theta_X < 0) > 0)) {
      V_theta_X <- diag(ginv(-Hessian))
    }
  ...
  ### Else perform the BFGS method directly on the entire portfolio
    ...
    ell_r[1] <- Q_N(theta_N = theta_N, Prob_Z_hat = Prob_Z_hat, 
                    Counts = Counts, Expo = Expo, RF_N = RF_N, LB_eps = LB_eps, penalty = penalty) +
      Q_X(theta_X = theta_X, Prob_Z_hat = Prob_Z_hat[Ind_i_t], 
    ...
    M_Q <- maxBFGS(fn = function(theta) Q_N_Sparse(theta_N = theta, Prob_Z_hat = Prob_Z_hat, 
                   ...
                   constraints = list(ineqA = cbind(
                     matrix(0, nrow = 2, ncol = ncol(RF_N)), diag(1, nrow = 2, ncol = 2)), 
                     ineqB = rep(0, 2)))
    # Store the resulting solution
    ell_r[2] <- M_Q$maximum
    ...
    theta_N <- M_Q$estimate
    # Ensure constraints are not violated
    theta_N[(length(theta_N) - 1):length(theta_N)] <- pmax(theta_N[(length(theta_N) - 1):length(theta_N)], LB_eps)
    # Final variance estimates
    ...
    M_Q <- maxBFGS(fn = function(theta) Q_X_Sparse(theta_X = theta, Prob_Z_hat = Prob_Z_hat[Ind_i_t], 
                   ...
                   start = theta_X, control = list(tol = Tol_abs_M, reltol = Tol_rel_M, iterlim = Iter_M, 
                                                   printLevel = 0), finalHessian = TRUE, 
                   constraints = list(ineqA = rbind(c(1, rep(0, length(theta_X) - 1)), cbind(
                     matrix(0, nrow = 2, ncol = 1 + ncol(RF_X)), diag(1, nrow = 2, ncol = 2))), 
                     ineqB = c(0, -1, 0)))
    ...
    theta_X <- M_Q$estimate
    # Ensure constraints are not violated
    theta_X[c(1, length(theta_X))] <- pmax(theta_X[c(1, length(theta_X))], LB_eps)
    theta_X[length(theta_X) - 1] <- max(theta_X[length(theta_X) - 1], 1 + LB_eps)
    # Final variance estimates
